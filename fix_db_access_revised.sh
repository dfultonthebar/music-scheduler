...
# Script to fix database access issues for music_user on musicu-server (revised)
# Date: Wed Jun 04 19:00 CDT 2025

LOG_FILE="/music_scheduler/logs/fix_db_access_revised.log"

echo "Starting fix_db_access_revised.sh at $(date)" | tee -a "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "Debug: Logging initialized for fix_db_access_revised.sh"
...
# Step 3: Check root user authentication method
echo "Checking root user authentication method..." | tee -a "$LOG_FILE"
sudo mysql -e "SELECT user, host, plugin FROM mysql.user WHERE user = 'root';" 2>&1 | tee -a "$LOG_FILE"
if [ $? -ne 0 ]; then
    echo "Error: Failed to authenticate as root user with sudo mysql" | tee -a "$LOG_FILE"
    echo "Attempting to set a password for root user and disable unix_socket plugin..." | tee -a "$LOG_FILE"
    sudo systemctl stop mariadb 2>&1 | tee -a "$LOG_FILE"
    sudo mysqld_safe --skip-grant-tables & 2>&1 | tee -a "$LOG_FILE"
    sleep 5
    mysql -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'RootPassword2025'; FLUSH PRIVILEGES;" 2>&1 | tee -a "$LOG_FILE"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to set root password" | tee -a "$LOG_FILE"
        exit 1
    fi
    sudo systemctl start mariadb 2>&1 | tee -a "$LOG_FILE"
    mysql -u root -pRootPassword2025 -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'RootPassword2025' PASSWORD EXPIRE NEVER; SET GLOBAL authenticate_with_unix_socket = 0; FLUSH PRIVILEGES;" 2>&1 | tee -a "$LOG_FILE"
    if [ $? -ne 0 ]; then
        echo "Error: Failed to configure root user" | tee -a "$LOG_FILE"
        exit 1
    fi
fi

# Step 4: Create music_user and music_scheduler database using root with password
...
# Step 5: Insert initial users with hashed passwords
...
# Step 7: Verify Gunicorn is listening on 127.0.0.1:8000
...
# Step 8: Test API endpoints with curl
