...
mysql -umusicu_user -pMusicU2025 musicu_db > /music_scheduler/logs/create_tables.log 2>&1 <<EOF
CREATE TABLE users (
...
if [ $? -ne 0 ]; then
    echo "Error: Failed to create database tables. Check /music_scheduler/logs/create_tables.log for details."
    cat /music_scheduler/logs/create_tables.log
    exit 1
fi
...
# Clear dependent tables to avoid foreign key constraint issues
echo "Clearing dependent tables..."
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM instructor_availability;" 2>/dev/null || {
    echo "Warning: Failed to clear instructor_availability table (table might not exist)"
}
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM instructor_instruments;" 2>/dev/null || {
    echo "Warning: Failed to clear instructor_instruments table (table might not exist)"
}
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM instructor_time_off;" 2>/dev/null || {
    echo "Warning: Failed to clear instructor_time_off table (table might not exist)"
}
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM lessons;" 2>/dev/null || {
    echo "Warning: Failed to clear lessons table (table might not exist)"
}
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM students;" 2>/dev/null || {
    echo "Warning: Failed to clear students table (table might not exist)"
}

# Clear the users table
echo "Clearing users table..."
mysql -umusicu_user -pMusicU2025 musicu_db -e "DELETE FROM users;" 2>/dev/null || {
    echo "Warning: Failed to clear users table (table might not exist or be inaccessible)"
    # Check if the table exists
    TABLE_EXISTS=$(mysql -umusicu_user -pMusicU2025 musicu_db -e "SHOW TABLES LIKE 'users';" 2>/dev/null | grep users | wc -l)
    if [ "$TABLE_EXISTS" -eq 0 ]; then
        echo "Error: 'users' table does not exist. Check previous steps."
        exit 1
    else
        echo "Error: Failed to delete from users table despite table existing."
        exit 1
    fi
}
