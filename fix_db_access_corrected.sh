...
# Script to fix database access issues for music_user on musicu-server (corrected)
# Date: Wed Jun 04 19:10 CDT 2025

LOG_FILE="/music_scheduler/logs/fix_db_access_corrected.log"

echo "Starting fix_db_access_corrected.sh at $(date)" | tee -a "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "Debug: Logging initialized for fix_db_access_corrected.sh"
...
# Step 4: Set root password by updating mysql.user table
echo "Setting root password by updating mysql.user table..." | tee -a "$LOG_FILE"
mysql -e "UPDATE mysql.user SET authentication_string = PASSWORD('RootPassword2025'), plugin = 'mysql_native_password' WHERE User = 'root' AND Host = 'localhost'; FLUSH PRIVILEGES;" 2>&1 | tee -a "$LOG_FILE"
if [ $? -ne 0 ]; then
