...
# Script to fix database access issues for music_user on musicu-server (final revision)
# Date: Wed Jun 04 19:05 CDT 2025

LOG_FILE="/music_scheduler/logs/fix_db_access_final.log"

echo "Starting fix_db_access_final.sh at $(date)" | tee -a "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "Debug: Logging initialized for fix_db_access_final.sh"
...
# Step 3: Start MariaDB in safe mode to set root password
echo "Starting MariaDB in safe mode to set root password..." | tee -a "$LOG_FILE"
...
# Step 4: Set root password and disable unix_socket plugin
echo "Setting root password and disabling unix_socket plugin..." | tee -a "$LOG_FILE"
...
# Step 6: Configure root user to use password authentication
echo "Configuring root user to use password authentication..." | tee -a "$LOG_FILE"
...
# Step 7: Create music_user and music_scheduler database
...
# Step 8: Insert initial users with hashed passwords
...
# Step 9: Restart Gunicorn service
...
# Step 10: Verify Gunicorn is listening on 127.0.0.1:8000
...
# Step 11: Test API endpoints with curl
