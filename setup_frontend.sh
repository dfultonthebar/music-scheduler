...
rm -rf /root/.cache  # Clear root cache
rm -rf /music_scheduler/node_modules/.cache  # Clear local node_modules cache
rm -rf ~/.cache/tailwindcss  # Clear Tailwind CSS cache
...
# Force update of caniuse-lite and browserslist to suppress Browserslist warning
echo "Forcing update of caniuse-lite and browserslist..."
...
# Clear caches again after updating
rm -rf /music_scheduler/node_modules/.cache
rm -rf ~/.cache/tailwindcss
rm -rf /root/.cache
...
# Download favicon.ico with retries
echo "Downloading favicon.ico..."
for ATTEMPT in {1..3}; do
    curl -o /music_scheduler/static/favicon.ico https://www.google.com/favicon.ico
    if [ $? -eq 0 ] && [ -f /music_scheduler/static/favicon.ico ]; then
        echo "favicon.ico downloaded successfully."
        break
    else
        echo "Failed to download favicon.ico (Attempt $ATTEMPT/3)"
        if [ $ATTEMPT -eq 3 ]; then
            echo "Error: Failed to download favicon.ico after 3 attempts."
            exit 1
        fi
        sleep 2
    fi
