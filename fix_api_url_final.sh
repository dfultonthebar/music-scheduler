...
echo "Starting fix_api_url_final.sh logging at $(date)"
exec > >(tee -a /music_scheduler/logs/fix_api_url_final.log) 2>&1

echo "Debug: Logging initialized for fix_api_url_final.sh"

# Ensure we're in the correct directory
cd /music_scheduler

# Step 1: Update Node.js to a compatible version
...
# Step 2: Clear npm cache and update dependencies
echo "Clearing npm cache and updating dependencies..."
...
for script in setup_admin.sh setup_instructor.sh; do
    ...
    # Replace API_BASE_URL placeholder with the correct URL, escaping special characters
...
# Add the fix for timedelta serialization in manage_availability
...
echo "Fix complete! Check the logs for details: cat /music_scheduler/logs/fix_api_url_final.log"
