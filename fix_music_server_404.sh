...
# Script to fix the 404 error on musicu-server for internal testing
# Date: Wed Jun 04 16:45 CDT 2025

LOG_FILE="/music_scheduler/logs/fix_music_server_404.log"

echo "Starting fix_music_server_404.sh at $(date)" | tee -a "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "Debug: Logging initialized for fix_music_server_404.sh"

# Step 1: Ensure iptables-persistent is installed
echo "Ensuring iptables-persistent is installed..." | tee -a "$LOG_FILE"
...
# Step 2: Verify iptables rules for internal access
...
# Step 3: Fix Nginx configuration to resolve 404 error
...
echo "Ensuring symlink in /etc/nginx/sites-enabled/..." | tee -a "$LOG_FILE"
# Remove any existing symlink to avoid duplicates
rm -f /etc/nginx/sites-enabled/music_scheduler 2>&1 | tee -a "$LOG_FILE"
# Remove the default site if it exists
rm -f /etc/nginx/sites-enabled/default 2>&1 | tee -a "$LOG_FILE"
