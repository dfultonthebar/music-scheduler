...
# Script to fix database access issues for music_user on musicu-server (MariaDB 10.11.11 compatible)
# Date: Wed Jun 04 19:15 CDT 2025

LOG_FILE="/music_scheduler/logs/fix_db_access_mariadb.log"

echo "Starting fix_db_access_mariadb.sh at $(date)" | tee -a "$LOG_FILE"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "Debug: Logging initialized for fix_db_access_mariadb.sh"
...
mysql -e "UPDATE mysql.user SET plugin = 'mysql_native_password', Password = 'RootPassword2025' WHERE User = 'root' AND Host = 'localhost'; FLUSH PRIVILEGES;" 2>&1 | tee -a "$LOG_FILE"
if [ $? -ne 0 ]; then
